...
import { Edit, Trash2, CheckCircle, Grid, Search, Maximize, Minimize } from 'lucide-react';
    ...
    <div className="bg-[#F7F7F7] min-h-[calc(100vh-100px)] grid grid-cols-1 lg:grid-cols-3 gap-4">
      ...
      <div className="lg:col-span-1 bg-white border border-[#E6E9ED] shadow-sm rounded-sm">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-5 flex flex-col gap-4">
          <div>
            <label className="block text-[13px] font-bold text-black font-bold mb-1">Brand Name *</label>
            <input 
              {...register('name')}
              type="text" 
          ...
          <div>
            <label className="block text-[13px] font-bold text-black font-bold mb-1">Parent Brand (Under)</label>
            <select 
              {...register('parentId')}
              ...
              {isLoading ? (
                ...
                filteredBrands.map((brand: any, index: number) => (
                  <tr key={brand.id} className={`border-b border-[#E5E7EB] ${index % 2 === 0 ? 'bg-[#F9F9F9]' : 'bg-white'} hover:bg-blue-50`}>
                    ...
                    <td data-label="Parent Brand" className="px-3 py-2.5 border-r border-[#E5E7EB] text-black font-bold">{brand.parent ? brand.parent.name : '-'}</td>
