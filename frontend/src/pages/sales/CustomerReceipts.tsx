...
const CustomerReceipts = () => {
  ...
  const { register, handleSubmit, watch, setValue, reset, } = useForm<ReceiptFormValues>({
              ...
              <div className="flex flex-col gap-3">
                <div className="border border-[#CBD5E1] rounded-lg p-3 flex justify-between items-center bg-white">
                  <div className="text-[13px] font-bold text-[#475569]">
                    Over All Outstanding Balance: <span className="text-[#059669] text-[15px]">RM {currentBalance.toFixed(2)}</span>
                  </div>
                  <button 
                    type="button"
                    onClick={() => setShowBreakdown(!showBreakdown)}
                    className="border-2 border-[#10B981] text-[#059669] px-3 py-1.5 rounded font-bold text-[13px] hover:bg-[#ECFDF5] flex items-center gap-2 transition-colors"
                  >
                ...
                {showBreakdown && (
                  <div className="border border-[#10B981] rounded-lg overflow-hidden bg-white">
                    <div className="bg-[#10B981] text-white px-3 py-2 flex justify-between items-center text-[12px] font-bold">
                      <div className="flex items-center gap-2"><FileText size={14} /> BILL-BY-BILL OUTSTANDING BREAKDOWN</div>
                      <span className="bg-white text-[#10B981] px-2 py-0.5 rounded-full text-[10px]">{unpaidBills.length} Bills</span>
                    </div>
                        ...
                        <tbody>
                          {unpaidBills.length > 0 ? unpaidBills.map((bill, idx) => (
                            <tr key={idx} className="border-b border-[#E2E8F0] hover:bg-[#F8FAFC]">
                              ...
                              <td className="px-3 py-2 text-right font-bold text-[#059669]">RM {bill.pending.toFixed(2)}</td>
                            </tr>
                          )) : (
                            <tr>
